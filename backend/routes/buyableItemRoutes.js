 ...
 */
 router.post("/ticket", async (req, res) => {
  try {
    ...
    res.status(400).send(`An error occurred while creating a ticket for a tourist attraction! Error => ${message}`);
  }
});
 ...
 */
 router.post("/menu-item", async (req, res) => {
  try {
    ...
    res.status(400).send(`An error occurred while creating a menu item for a restaurant! Error => ${message}`);
  }
});
 ...
 */
 router.put("/ticket/:id", async (req, res) => {
  try {
    ...
    const ticketUpdated = await buyableItemController.updateTicket(ticketId, query);
    res.status(200).send(ticketUpdated);
  } catch ({ message }) {
    res.status(400).send(`An error occurred while updating a ticket! Error => ${message}`);
  }
});
 ...
 */
 router.put("/menu-item/:id", async (req, res) => {
  try {
    ...
    const menuItemUpdated = await buyableItemController.updateMenuItem(menuItemId, query);
    res.status(200).send(menuItemUpdated);
  } catch ({ message }) {
    res.status(400).send(`An error occurred while updating a menu item! Error => ${message}`);
  }
});
 ...
 */
 router.delete("/ticket/:id", async (req, res) => {
  try {
    const ticketRemoved = await buyableItemController.deleteTicket(req.params.id);
    ...
    res.status(400).send(`An error occurred while deleting the ticket! Error => ${message}`);
  }
});
 ...
 */
 router.delete("/menu-item/:id", async (req, res) => {
  try {
    const menuItemRemoved = await buyableItemController.deleteMenuItem(req.params.id);
    ...
    res.status(400).send(`An error occurred while deleting the menu item! Error => ${message}`);
  }
});
 ...
 */
 router.post("/", async (req, res) => {
  try {
    ...
    res.status(400).send(`An error occurred while getting buyable items of partner locations! Error => ${message}`);
  }
});
